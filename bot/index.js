...
class TelegramBot {
    ...
    async setupCommands() {
        ...
        this.bot.action('mode_free', async (ctx) => {
            try {
                ...
                await this.alertAdmins(error, { userId: ctx.from?.id, updateType: 'callback_query', command: 'mode_free', source: 'action.mode_free', note: 'Free mode entry crash' });
            }
        });
        ...
        this.bot.action('help', async (ctx) => {
            try {
                ...
                await this.alertAdmins(error, { userId: ctx.from?.id, updateType: 'callback_query', command: 'help', source: 'action.help', note: 'Help action crash' });
            }
        });
        ...
        this.bot.action('menu', async (ctx) => {
            try {
                ...
                await this.alertAdmins(error, { userId: ctx.from?.id, updateType: 'callback_query', command: 'menu', source: 'action.menu', note: 'Menu action crash' });
            }
        });
        ...
        this.bot.action('open_admin_dashboard', async (ctx) => {
            try {
                ...
                await this.alertAdmins(err, { userId: ctx.from?.id, updateType: 'callback_query', command: 'open_admin_dashboard', source: 'action.openAdminDashboard', note: 'Admin dashboard action crash' });
            }
        });
    }
    ...
    setupTextHandler() {
        this.bot.on(message('text'), async (ctx, next) => {
            try {
                ...
                await this.alertAdmins(err, { userId: ctx.from?.id, updateType: 'message', command: ctx.message?.text, source: 'handler.textMessage', note: 'Text handler crash' });
            }
        });
    }
    ...
    startDepositScanner() {
        ...
        const checkAndStart = async () => {
            ...
            try {
                ...
                await this.alertAdmins(error, { source: 'scanner.depositScanner', note: 'Deposit scanner retry loop error' });
                setTimeout(checkAndStart, retryDelay);
                retryDelay = Math.min(retryDelay * 2, maxRetryDelay);
            }
        };
    ...
    async launch() {
        try {
            ...
            await this.alertAdmins(error, { source: 'launch.botLaunch', note: 'Bot failed to launch — critical' });
            throw error;
        }
    }
